...
def get_token_auth_header():
    ...
    if len(authDetails) != 2:  # Auth header must be two parts; type and token
        raise AuthError({
            'code': 'auth_header_invalid',
            'description': 'Authorization header must \
                contain the token type and the token.'
        }, 401)
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:  # Check for 'permissions' key
        raise AuthError({
            'code': 'invalid_permission',
            'description': 'Permission was not specified in the token.'
        }, 401)

    if permission not in payload['permissions']:  # Check for permission
        raise AuthError({
            'code': 'unauthorized',
